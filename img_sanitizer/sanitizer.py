...
class Sanitizer:
    ...
    def __init__(self, source: Path, dest: Path, worker: int) -> None:
    ...
    def _sha1_file(self, filepath: Path, buffer_size: int = 65536) -> str:
        ...
        sha1 = h.hexdigest()
        img_sanitizer.logger.info("SHA1 of %s : %s", filepath, sha1)
        return sha1
